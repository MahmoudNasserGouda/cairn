...
Rujoom targets students, junior developers, and developers in emerging
...
Rujoom-owned AI inference bill, no mandatory database, no mandatory
application backend, and maximum browser/device computation.

## Decision

We will build Rujoom as a **local-first** application. The core product —
...
storage. No Rujoom-owned infrastructure is required for the core experience.
