...
Gemini, and OpenRouter, no OSC-funded inference, and a useful non-AI mode.
...
- The user supplies their own API key; requests go **directly from the browser to the
  chosen provider** (or via the provider's own proxy, never OSC's).
...
- $0 AI cost to OSC; the user controls spend and model choice.
...
- CORS: not all providers allow direct browser calls with all key types; where a
  ...
  browser-allowed path — do **not** add an OSC proxy.
...
- **OSC-hosted inference with a free tier.** Rejected: unbounded cost, the central risk
