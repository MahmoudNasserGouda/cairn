# Open Source Compass — Architecture Overview

This is the entry point for the Open Source Compass architecture. It gives the system
...
Open Source Compass helps students, junior developers, and developers in emerging
...
| **$0 operating cost for the MVP** | Static hosting, no backend, no DB, no OSC AI bill | [0001](docs/adr/0001-local-first-zero-cost-architecture.md), [0004](docs/adr/0004-static-first-web-app-on-free-hosting.md) |
...
flowchart TB
    ...
    subgraph Clients["Open Source Compass clients"]
...
sequenceDiagram
    ...
    U->>D: Send
    App->>Prov: direct request with user's key (never via OSC)
    Prov-->>App: response
...
| **Secrets** | GitHub token, BYOK AI keys | Memory by default; encrypted IndexedDB / isolated store opt-in | Never logged, never to OSC, never in URLs ([ADR-0010](docs/adr/0010-ai-key-privacy-and-data-disclosure.md), [ADR-0020](docs/adr/0020-oauth-token-and-byok-key-handling.md)) |
